...
# This script runs ghcp directly without installing
#
set -e
usage() {
  this=$1
...
configure_from_env() {
  ...
  if [ -n "${BINSTALLER_TARGET_TAG}" ]; then
    log_crit "BINSTALLER_TARGET_TAG is set but this script is configured for v1.15.0 only"
...
resolve_asset_filename() {
  ...
  if [ -z "${ASSET_FILENAME}" ]; then
    ASSET_FILENAME="ghcp_${OS}_${ARCH}${EXT}"
  fi
}
